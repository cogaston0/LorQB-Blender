...
# C14: Ball transfers from Green to Yellow
# Green rotates 180° around Hinge_Green_Yellow (left edge, X-axis)
# Red parented to Green — passively swings above Blue at 180°
# Frame range: 481 — 720
...
################################################################################
# SECTION 5: Main C14 setup function
################################################################################
def setup_green_to_yellow():
    ...
    # --- 5C: Parent Red to Green (passive carry) ---
    if red.parent != green:
        parent_preserve_world(red, green)
        print("Red parented to Green — passive carry.")

    # --- 5D: Parent Green to Hinge_Green_Yellow ---
    if green.parent != hinge_gy:
        parent_preserve_world(green, hinge_gy)
        print("Green parented to Hinge_Green_Yellow.")

    # --- 5E: Create Seat_Green (ball rest position inside Green) ---
    # Created here so C14 works standalone without depending on C13
    ...
    # --- 5F: Create Seat_Yellow (ball rest position inside Yellow) ---
    ...
    # --- 5H: Position ball at Seat_Green at frame F_START ---
    bpy.context.scene.frame_set(F_START)
    latch_green.influence  = 1.0
    latch_yellow.influence = 0.0

    # --- 5I: Keyframe hinge rotation X-axis ---
    ...
    print(f"Red above Blue at frame {F_HOLD}")
...
class LORQB_PT_C14Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        self.layout.operator("lorqb.green_to_yellow", text="Run C14: Green → Yellow")
